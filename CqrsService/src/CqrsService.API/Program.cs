...
using CqrsService.API.Filters;
using CqrsService.API.Options;
...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("ApplicationName", "CqrsService.API")
...
builder.Services.AddSwaggerGen(options =>
{
    ...
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "CqrsService", Version = "v1", Description = "Description" });
    options.SwaggerDoc("v2", new OpenApiInfo { Title = "CqrsService", Version = "v2", Description = "Description" });
...
// Enable to use kafka event streaming
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "CqrsService V1");
    c.SwaggerEndpoint("/swagger/v2/swagger.json", "CqrsService V2");
    c.RoutePrefix = string.Empty;
});

try
{
    Log.Information("Starting CqrsService API");
    app.Run();
}
catch (Exception e)
{
    Log.Fatal(e, "Unable to start CqrsService API");
}
finally
{
    Log.CloseAndFlush();
