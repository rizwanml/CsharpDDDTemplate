...
public sealed class AzureServiceBusProvider : IAzureServiceBusProvider
{
    ...
    public AzureServiceBusProvider(ServiceBusClientOptions clientOptions)
    {
    ...
    public async Task Send<TModel>(string connectionString, string queueName, TModel message, AzureServiceBusMessageOptions? options = null) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(AzureServiceBusProvider), methodName: nameof(Send), operationDetail: queueName);
    ...
    public async Task<IReadOnlyList<AzureServiceBusReceiveResponse<TModel>>> Receive<TModel>(string connectionString, string queueName, string? identifier = null, int? prefetchCount = null, int maxNumberOfMessages = 10, int waitTimeSeconds = 20) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(AzureServiceBusProvider), methodName: nameof(Receive), operationDetail: queueName);
            ExceptionDispatchInfo.Capture(ex).Throw();
            throw; //to satisfy the compiler
        }
    }
    ...
    public async Task<bool> Delete(string connectionString, string queueName, long messageId)
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(AzureServiceBusProvider), methodName: nameof(Delete), operationDetail: queueName);
            ExceptionDispatchInfo.Capture(ex).Throw();
            throw; //to satisfy the compiler
        }
    }
    ...
    public async Task BatchDelete(string connectionString, string queueName, IReadOnlyList<AzureServiceBusBatchDeleteRequest> azureServiceBusBatchDeleteRequest)
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(AzureServiceBusProvider), methodName: nameof(BatchDelete), operationDetail: queueName);
            ExceptionDispatchInfo.Capture(ex).Throw();
            throw; //to satisfy the compiler
        }
    }
