...
public sealed class RabbitMqProvider : IRabbitMqProvider
{
    ...
    public async Task Send<TModel>(string queueName, TModel model, Dictionary<string, string> messageAttributes = default) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(RabbitMqProvider), methodName: nameof(Send), operationDetail: queueName);
    ...
    public async Task<TModel> Receive<TModel>(string queueName) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(RabbitMqProvider), methodName: nameof(Receive), operationDetail: queueName);
            ExceptionDispatchInfo.Capture(ex).Throw();
            throw; //to satisfy the compiler
        }
    }
