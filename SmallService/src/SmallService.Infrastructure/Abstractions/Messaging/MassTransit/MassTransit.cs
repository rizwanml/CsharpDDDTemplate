...
public sealed class MassTransit : IMassTransit
{
    private readonly IBusControl _busControl;
    private readonly IPublishEndpoint _publishEndpoint;
    private readonly IMessageScheduler _messageScheduler;

    public MassTransit(IBusControl busControl, IPublishEndpoint publishEndpoint, IMessageScheduler messageScheduler)
    {
    ...
    public async Task Send<TModel>(string queueName, TModel model, TimeSpan? delay = null) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(MassTransit), methodName: nameof(Send), operationDetail: queueName);
    ...
    public async Task Publish<TModel>(TModel model, TimeSpan? delay = null) where TModel : class
    {
        ...
        {
            ExceptionHandlerHelpers.HandleException(exception: ex, className: nameof(MassTransit), methodName: nameof(Publish));
            ExceptionDispatchInfo.Capture(ex).Throw();
            throw; //to satisfy the compiler
        }
    }
